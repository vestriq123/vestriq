...
export const POST = apiHandler(async (request: Request, context: unknown) => {
  ...
  if (deposit.status !== DepositStatus.PENDING) {
    return errorResponse("Only pending deposits can be rejected", 400);
  }

  const updatedDeposit = await depositRepository.reject(id);
  return successResponse(updatedDeposit, "Deposit request has been rejected");
});
